...
const validatePullRequest = async data => {
  ...
  if (data.action !== 'opened' && data.action !== 'reopened') {
    if (data.action === 'synchronize') {
      const msgTest = `@${data.sender.login} updated the pull request.`;
      console.log(msgTest);

      githubConfig.body = msgTest;
      github.issues.createComment(githubConfig);
      githubConfig.body = null;
    }
    return;
  }
  ...
  if (repoConfig.userForbiddenForPR) {
    ...
    if (msg) {
      ...
      githubConfig.base = data.pull_request.base.ref;
      github.pullRequests.update(githubConfig);
      delete githubConfig.state;
      delete githubConfig.base;
      return;
    }
  }
...
export default validatePullRequest;
